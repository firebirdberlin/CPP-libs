...
*/
#define		GT			1
#define		PIRIS		2		/*	Personal IRIS 24 plans + Z-buffer */
#define		PIRIS8		3		/*	Personal IRIS  8 plans */

/*
	Coprocesseurs mathematiques et accelerateurs pour PC-compatibles
*/
#define		I80x87	1

/*
	Coprocesseurs mathematiques et accelerateurs pour IRIS et/ou Sun
*/
#define		M6888x	1
#define		FPA		2
#define		SKY		3
...
/* We only care about big or little endedness. The other systemes are kept
 * for historical reasons												 */
#ifdef WORDS_BIGENDIAN
#define IGB_SYSTEME IGB_BIG_ENDIAN
#else
#define IGB_SYSTEME IGB_LITTLE_ENDIAN
#endif
...
template<class T>
int read_IGB_data( T* dp, int numt, IGBheader* h );
...
private:
	...
	char transstr[257];
	static std::string TypeDescription[IGB_MAX_TYPE+1];
...
public:
	...
	inline void dim(int x, int y=1, int z=1, int t=1){ this->x(x); this->y(y); this->z(z); this->t(t);}
	...
	inline void systeme( unsigned long a ){ v_systeme = a; bool_systeme=true; }
		   void systeme( char * );
	...
	size_t index(size_t ix,size_t iy=0,size_t iz=0,size_t it=0);
	static std::string TypeID2Name(int id);
	static int Name2TypeID(const std::string name);

	size_t slicesize() { return x()*y()*z();}
	size_t slices() { return t();}
	bool compatible(IGBheader& other);
	class Exception_IndexError :public myexception {
		public:	Exception_IndexError(std::string id) :
